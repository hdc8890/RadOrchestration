...
const os     = require('os');
const checkStructure = require('../validate/lib/checks/structure');
...
function setupFullGithub() {
  ...
  fs.mkdirSync(path.join(ghDir, 'skills', 'orchestration', 'config'), { recursive: true });
  fs.writeFileSync(path.join(ghDir, 'skills', 'orchestration', 'config', 'orchestration.yml'), 'projects:\n  base_path: custom/project-store/\n');
...
async function runTests() {
  ...
  try {
    ...
    await test('Pass results do NOT include a detail object', async () => {
      ...
      fs.mkdirSync(path.join(ghDir, 'prompts'));
      fs.mkdirSync(path.join(ghDir, 'skills', 'orchestration', 'config'), { recursive: true });
      fs.writeFileSync(path.join(ghDir, 'skills', 'orchestration', 'config', 'orchestration.yml'), 'config: true\n');
